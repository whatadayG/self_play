...
class OptimizationGame(DialogueGame):
    ...
    def __init__(self, env_config, one_player=False):
        self.TASKS = TASKS
    ...
    def create_from_game_state(cls, game_state, one_player=False):
        ...
        if game.one_player:
            # In one-player mode, create a combined table with both players' information
            # First, create a combined mask showing what either player knows
            combined_mask = np.logical_or(mask1, mask2)
            
            # Create a combined table with values from both players
            combined_table_values = game.table.values.copy()
            
            # For cells unknown to both players, use the baseline value (50)
            unknown_to_both = np.logical_not(combined_mask)
            combined_table_values[unknown_to_both] = 50
            
            # Create a table object with the combined values
            combined_table = Table(values=combined_table_values)
            
            # Create the unscaled table for one-player mode (scaling factor = 1.0)
            # Use all_visible_mask so that all values (including filled-in unknowns) are shown
            all_visible_mask = np.ones((num_rows, num_cols), dtype=bool)
            game.unscaled_table = game._preprocess_table(combined_table, all_visible_mask, 1.0)
            
            # Still keep the original tables for compatibility
            game.tables = [
                game._preprocess_table(game.table, game.masks[0], game.scales[0]),
                game._preprocess_table(game.table, game.masks[1], game.scales[1])
            ]
        else:
            # Two-player mode (default behavior)
            game.tables = [
                game._preprocess_table(game.table, game.masks[0], game.scales[0]),
                game._preprocess_table(game.table, game.masks[1], game.scales[1])
            ]
        ...
        # Populate historical action log **before** computing turn order so
        # that `turn_player` correctly reflects whose turn is next when
        # resuming from an offline state.
        game.action_log = game_state.get("action_log", [])

        # If there is at least one prior turn we infer the next speaker as the
        # *other* player relative to the most recent entry in the log.
        ...
        while True and randomize:
            ...
            if (rew1 * 1.25 < self.best_assignment_reward) \
                ...
                if self.one_player:
                    # In one-player mode, create a combined table with both players' information
                    # First, create a combined mask showing what either player knows
                    combined_mask = np.logical_or(known1, known2)
                    
                    # Create a combined table with values from both players
                    combined_table_values = self.table.values.copy()
                    ...
                    # Create the unscaled table for one-player mode (scaling factor = 1.0)
                    # Use all_visible_mask so that all values (including filled-in unknowns) are shown
                    all_visible_mask = np.ones((self.num_rows, self.num_cols), dtype=bool)
                    self.unscaled_table = self._preprocess_table(combined_table, all_visible_mask, 1.0)
                    
                    # Still create the regular tables for compatibility
