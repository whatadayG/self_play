...
class DialogueEnv:
    ...
    def _parse_message(
        ...
        if edited_prompt_propose:
            ...
            # Use DOTALL flag so that '.' matches newline characters, allowing multi-line messages to be fully captured
            m = re.match(
                r"(?P<vroom>0|1|all): \[(?P<mtype>\w+)\](?P<msg>.*)",
                message.strip(),
                flags=re.DOTALL)
        else:
            # Use DOTALL flag here as well for multi-line message support
            m = re.match(
                r"\[(?P<mtype>\w+)\](?P<msg>.*)",
                message.strip(),
                flags=re.DOTALL)
