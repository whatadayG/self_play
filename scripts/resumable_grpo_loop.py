...
def analyze_run_state(save_root: Path) -> Tuple[int, str, Optional[str], Dict[str, Any]]:
    ...
    progress_log = latest_round_dir / "progress.log"
    state_info = {"round_dir": latest_round_dir}
    
    # Parse progress log to understand current state
    events = []
    ...
    # Determine current model from previous rounds or events
    current_model = None
    
    # Look for the most recent model from completed rounds
    if round_num > 0:
    ...
    # Check if rollout phase is complete
    rollout_complete = any(e.get("event") == "generation_done" for e in events)
    trim_complete = any(e.get("event") == "trim_done" for e in events)
    sft_complete = any(e.get("event") == "sft_done" for e in events)
    round_complete = any(e.get("event") == "round_complete" for e in events)
    
    if round_complete:
        # This round is complete, start next round
        # Get the next model from the completed round
        for event in reversed(events):
            if event.get("event") == "round_complete":
                current_model = event.get("next_model")
                break
        return round_num + 1, 'rollout', current_model, state_info
    elif sft_complete:
        # SFT is complete but round not marked complete, finish the round
        return round_num, 'finish_round', current_model, state_info
    elif rollout_complete and trim_complete:
