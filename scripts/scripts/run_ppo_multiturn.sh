...
# Models
export POLICY_MODEL=${POLICY_MODEL:-Qwen/Qwen2-7B-Instruct}
...
# Build small parquet from jsonl
python RL-matching/scripts/build_matching_parquet.py \
  --input RL-matching/dialop/data/optimization.jsonl \
  --output RL-matching/data/matching.parquet

# Start GRPO training (2 GPUs) without @config include (Hydra)
python -m verl.trainer.main_ppo \
  ...
  reward_model.custom_reward_function.path=/home/nickatomlin/georgiazhou/new_dialop/RL-matching/verl_hooks/matching_reward_fn.py \
  reward_model.custom_reward_function.name=compute_score_matching \
  ...
  trainer.experiment_name=grpo_multiturn_lora \
