#!/bin/bash
# Run matching game RL training with verl
...
CUDA_VISIBLE_DEVICES=2,3,4,5 PYTHONUNBUFFERED=1 python3 -m verl.trainer.main_ppo \
    ...
    trainer.project_name='matching_game_rl' \
    trainer.experiment_name='qwen2.5-0.5B-matching' \
    trainer.n_gpus_per_node=4 \
    trainer.nnodes=1 \
    trainer.save_freq=10 \
    trainer.test_freq=10 \
    data.train_files=$PROJECT_DIR/data/matching_game/train.parquet \
    data.val_files=$PROJECT_DIR/data/matching_game/test.parquet \
